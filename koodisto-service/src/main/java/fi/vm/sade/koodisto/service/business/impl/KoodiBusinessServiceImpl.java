...
import fi.vm.sade.koodisto.service.business.exception.KoodistosHaveNoRelationException;
...
public class KoodiBusinessServiceImpl implements KoodiBusinessService {
    ...
    private static final Logger logger = LoggerFactory.getLogger(KoodiBusinessServiceImpl.class);
    ...
    public KoodiVersioWithKoodistoItem createKoodi(String koodistoUri, CreateKoodiDataType createKoodiData) {
        ...
        return createKoodi(koodistoUri, createKoodiData, koodistoVersio);
    }

    private KoodiVersioWithKoodistoItem createKoodi(String koodistoUri, CreateKoodiDataType createKoodiData, KoodistoVersio latestKoodistoVersio) {
    ...
    public KoodistoVersio massCreate(String koodistoUri, List<UpdateKoodiDataType> koodiList) {
        ...
        for (UpdateKoodiDataType data : koodisToBeUpdated) {
            ...
            KoodistoVersioKoodiVersio result = koodistoVersioKoodiVersioRepository
                    .findByKoodistoVersioIdAndKoodiVersioId(koodisto.getId(), updated.getKoodiVersio().getId())
                    .orElse(null);
            if (result == null) {
                ...
                koodistoVersioKoodiVersioRepository.save(newRelationEntry);
            }
        }
        ...
        for (CreateKoodiDataType createData : createKoodiList) {
            createKoodi(koodistoUri, createData, newKoodistoVersio);
        }
    ...
    private void addRelation(KoodiVersio latest, List<String> relatedCodeElements, SuhteenTyyppi st, boolean isChild) {
        ...
        if (!isChild || st.equals(SuhteenTyyppi.RINNASTEINEN)) { // SISALTAA OR RINNASTUU
            if (SuhteenTyyppi.SISALTYY.equals(st)) {
                latest = createNewKoodistoVersionIfNeeded(latest);
                FindOrCreateWrapper<KoodiVersio> wrapper = createNewVersion(latest);
            ...
            for (String koodistoUri : koodistos) {
                koodistoBusinessService.createNewVersion(koodistoUri).isCreated();
            }
            ylakoodiVersios = getLatestKoodiVersios(ylakoodiUris.toArray(new String[ylakoodiUris.size()]));
            for (KoodiVersioWithKoodistoItem latestYlakoodi : ylakoodiVersios) {
                createNewVersion(latestYlakoodi.getKoodiVersio()).isCreated();
            }
            ylakoodiVersios = getLatestKoodiVersios(ylakoodiUris.toArray(new String[ylakoodiUris.size()]));
    ...
    private void removeRelation(KoodiVersio latest, List<String> relatedCodeElements, SuhteenTyyppi st, boolean isChild) {
        ...
        if (!isChild || st.equals(SuhteenTyyppi.RINNASTEINEN)) { // SISALTAA OR RINNASTUU
            ...
            for (KoodiVersioWithKoodistoItem string : ylakoodiVersios) {
                ...
                if (!koodistos.contains(latestYlakoodiKoodisto)) {
                    koodistos.add(latestYlakoodiKoodisto);
                    koodistoBusinessService.createNewVersion(latestYlakoodiKoodisto).isCreated();
                }
            }
    ...
    private KoodiVersio updateOldVersion(KoodiVersio latest, UpdateKoodiDataType updateKoodiData) {
        ...
        outer: for (KoodiMetadataType updateMetadata : updateKoodiData.getMetadata()) {
            ...
            koodiMetadataRepository.save(newMetadata);
        }
    ...
    private KoodiVersio createNewVersion(UpdateKoodiDataType updateKoodiData, KoodistoVersio newKoodistoVersio) {
        ...
        koodistoVersioKoodiVersioRepository
                .findByKoodistoVersioIdAndKoodiVersioId(newKoodistoVersio.getId(), latest.getId())
                .ifPresent(relation -> {
        ...
        koodistoVersioKoodiVersioRepository.save(newRelation);

        return inserted;
    }

    private FindOrCreateWrapper<KoodiVersio> createNewVersion(KoodiVersio latest) {
        ...
        return FindOrCreateWrapper.created(createNewVersion(latest, newVersio));
    }
    ...
    private KoodiVersio createNewVersion(KoodiVersio base, KoodiVersio input) {
        ...
        KoodiVersio inserted = koodiVersioRepository.save(input);
    ...
    public KoodiVersio createNewVersion(String koodiUri) {
        return createNewVersion(getLatestKoodiVersio(koodiUri)).getData();
    }
    ...
    public Set<KoodiVersio> createNewVersionsNonFlushing(Set<KoodistoVersioKoodiVersio> koodiVersios) {
        ...
        for (KoodistoVersioKoodiVersio koodiVersio : koodiVersios) {
            inserted.add(this.createNewVersion(koodiVersio.getKoodiVersio()).getData());
        }
        return inserted;
    }
    ...
    private void setPreviousVersionEndDateToNowOrKeepFutureDate(KoodiVersio latest) {
        koodiVersioRepository.getPreviousKoodiVersio(latest.getKoodi().getKoodiUri(), latest.getVersio()).ifPresent(
                (previousVersion) -> previousVersion.setVoimassaLoppuPvm(
                        getValidEndDateForKoodiVersio(previousVersion, latest)));
    }
    ...
    public void acceptCodeElements(KoodistoVersio latest) {
        ...
        if (koodis.size() > 0) {
            ...
            for (Entry<KoodiVersio, Optional<KoodiVersio>> entry : previousVersions.entrySet()) {
                entry.getValue().ifPresent(previousVersion -> {
    ...
    public void delete(String koodiUri, Integer koodiVersio, boolean skipPassiivinenCheck) {
        ...
        if (kvkoodisto.getKoodistoItem().getVersios().contains(latestKoodistoVersio.getVersio())
                && latestKoodistoVersio.getTila().equals(Tila.HYVAKSYTTY)) {
            ...
            koodistoVersioKoodiVersioRepository
                    .findByKoodistoVersioIdAndKoodiVersioId(newKoodistoVersio.getId(), versio.getId())
                    .ifPresent(koodistoVersioKoodiVersioRepository::delete);
        } else {
            ...
            for (KoodistoVersio kv : koodistoVersios) {
                koodistoVersioKoodiVersioRepository
                        .findByKoodistoVersioIdAndKoodiVersioId(kv.getId(), versio.getId())
                        .ifPresent(kv::removeKoodiVersio);
            }
    ...
    public boolean hasRelationBetweenCodeElements(KoodiVersio ylaKoodiVersio, final KoodiVersio alaKoodiVersio) {
        return Iterables.tryFind(ylaKoodiVersio.getAlakoodis(),
                input -> input.getAlakoodiVersio().equals(alaKoodiVersio)).isPresent();
    }
